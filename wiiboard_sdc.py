...
class EventProcessor:
    ...
    def mass(self, event):
        if (event.totalWeight > 2):
            print "spot weight is %s and most common is %d and have %d samples" % (event.totalWeight, self.weight, self._measureCnt)
...
class Wiiboard:
    def __init__(self, processor):
        ...
        self.currentWeight = 0 # so we can look for jumps
    ...
    def reportWeight(self):
        self.lastReport = datetime.now()
        print "reported weight"

    def receive(self):
        while self.status == "Connected" and not self.processor.done:
            #print "in da receive loop son!"
            if (datetime.now() - self.lastReport).seconds > REPORTING_INTERVAL:
                self.reportWeight()
